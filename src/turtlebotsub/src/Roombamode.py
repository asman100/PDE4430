...
locations = [(0.5, 0.5, False), (0.5, 3, False), (0.5, 5.5, False), (0.5, 8, False), (0.5, 10.5, False)]
unused_locations = []
start_locations = []
Roomba1_loc = Pose()
Roomba2_loc = Pose()
...
def find_start(bot_location):
    start_locations = locations.copy() 
    unused_locations = [target for target in start_locations if not target[2]]
    
    if not unused_locations:
        return None

    distances = [np.sqrt((bot_location[0] - target[0]) ** 2 + (bot_location[1] - target[1]) ** 2) for target in unused_locations]
    min_distance_index = np.argmin(distances)
    start_point = unused_locations[min_distance_index]
    start_locations[start_locations.index(start_point)] = (start_point[0], start_point[1], True)

    return start_point


def locationname(target_location):
    for index, location in enumerate([(0.5, 0.5), (0.5, 3), (0.5, 5.5), (0.5, 8), (0.5, 10.5)]):
        if location == target_location:
            return index
    return -1 

def Roomba1_data(data1, Roomba1): 
    Roomba1_loc = data1
...
def Roomba2_data(data2, Roomba2): 
    Roomba2_loc = data2
...
def Roomba3_data(data3, Roomba3): 
    Roomba3_loc = data3
...
def Roomba4_data(data4, Roomba4): 
    Roomba4_loc = data4
...
def Roomba5_data(data5, Roomba5): 
    Roomba5_loc = data5
...
def roombamode():
    ...
    rospy.init_node('Roombamode', anonymous=True)

    Roomba1 = rospy.Publisher('/Roomba_Helper_1', Int32, queue_size=10)
    rospy.Subscriber('/turtle1/pose', Pose, Roomba1_data, Roomba1)
